function OUT = plot_REM_6h_baseline_vs_drugs_APPvsWT(ALL_SUMMARY_6h, out_dir, varargin)
% plot_REM_6h_baseline_vs_drugs_APPvsWT
% -------------------------------------------------------------------------
% For the 6 h CROPPED windows (baseline + drugs), compare REM metrics:
%
...
%   4 bars: WT-baseline, WT-drugs, APP-baseline, APP-drugs
%
% Stats:
%   - Within-genotype paired t-test (baseline vs drugs), WT & APP
...
T = T(T.cond=="baseline" | T.cond=="drugs", :);
if isempty(T)
    warning('No baseline + drugs rows in ALL_SUMMARY_6h.');
...
if height(T) < 2
    warning('Too few recordings with both baseline & drugs.');
    OUT = struct('success',false); return;
end

% Metrics to plot: {field, y_label, scale_factor, nice_name}
metrics = {
    'total_REM_dur_s',          'Total REM time (min, 6 h window)',  1/60, 'totalREMmin';
...
for m = 1:size(metrics,1)
    ...
    if ~ismember(field, T.Properties.VariableNames)
        warning('Metric %s missing in ALL_SUMMARY_6h. Skipping.', field);
        continue;
    end
    ...
    % wide per mouse × geno: baseline / drugs
    Twide = unstack(Ti(:,{'mouse','geno','cond','Y'}), 'Y','cond');
    if ~all(ismember({'baseline','drugs'}, Twide.Properties.VariableNames))
        warning('Metric %s: not all mice have both baseline & drugs.', field);
    end

    % containers for bar means / SEMs
    meanMat = nan(4,1);  % WT_b, WT_d, APP_b, APP_d
    semMat  = nan(4,1);
    ...
    for gi = 1:numel(genotypes)
        ...
        if ~all(ismember({'baseline','drugs'}, Tg.Properties.VariableNames))
            continue;
        end

        base = Tg.baseline;
        drugs = Tg.drugs;
        ok   = ~isnan(base) & ~isnan(drugs);
        base = base(ok); drugs = drugs(ok);
        nPairs = numel(base);
        if nPairs < minN, continue; end

        [~, p_pair] = ttest(base, drugs);

        m_base  = mean(base,'omitnan');
        m_drugs  = mean(drugs,'omitnan');
        se_base = std(base,'omitnan')/sqrt(nPairs);
        se_drugs = std(drugs,'omitnan')/sqrt(nPairs);
        delta   = drugs - base;

        p_within(gi,m) = p_pair;

        paired_stats = [paired_stats; ...
            {char(gtype), nPairs, m_base, m_drugs, mean(delta,'omitnan'), p_pair}]; %#ok<AGROW>

        if gtype=="WT"
            meanMat(1) = m_base;  semMat(1) = se_base;
            meanMat(2) = m_drugs;  semMat(2) = se_drugs;
        else
            meanMat(3) = m_base;  semMat(3) = se_base;
            meanMat(4) = m_drugs;  semMat(4) = se_drugs;
        end
    end

    % Mixed-effects ANOVA: Y ~ cond * geno + (1|mouse)
    Ti2 = Ti;
    Ti2.cond  = categorical(Ti2.cond, {'baseline','drugs'});
    ...
    x = 1:4;
    labels = {'WT base','WT drugs','APP base','APP drugs'};
    barWidth = 0.6;
    ...
    % stars (using raw p's for the visual, FDR stored separately)
    for gi = 1:numel(genotypes)
        gtype = genotypes(gi);
    ...
    ylabel(y_label);
    title(sprintf('6 h REM metric: %s', field), 'Interpreter','none');
    set(gca,'Box','off');

    fig_file = fullfile(out_dir, sprintf('REM6h_%s_baseline_vs_drugs_APPvsWT.png', short_id));
    saveas(gcf, fig_file);
    ...
    if ~isempty(within_tbl)
        writetable(within_tbl, ...
            fullfile(out_dir, sprintf('REM6h_%s_within_stats.csv', short_id)));
    end
    writetable(anova_tbl, ...
        fullfile(out_dir, sprintf('REM6h_%s_anova_stats.csv', short_id)));
end
...
OUT.success = true;
fprintf('✅ REM 6 h bar plots + stats saved in %s\n', out_dir);
end
