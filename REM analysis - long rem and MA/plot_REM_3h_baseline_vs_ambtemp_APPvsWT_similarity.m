function OUT = plot_REM_3h_baseline_vs_ambtemp_APPvsWT_similarity(ALL_SUMMARY_3h, out_dir, varargin)
...
%
% Stats:
%   (1) Within-genotype paired t-test (baseline vs ambtemp), WT & APP
%       • BH-FDR across metrics per genotype
%       • stars between the 2 bars (per genotype)
%
%   (2) Mixed-effects model per metric:
%       metric ~ cond * geno + (1|mouse)
%       => p(cond), p(geno), p(cond×geno) printed on the plot
%
%   (3) NEW: per-condition WT vs APP comparison:
%       - baseline only:   unpaired t-test WT vs APP  -> p_base, Cohen d_base
%       - ambtemp only:    unpaired t-test WT vs APP  -> p_amb,  Cohen d_amb
%       These are printed on the figure and saved in CSV:
%         REM3h_<metric>_geno_cond_stats.csv
% -------------------------------------------------------------------------
...
% Metrics to plot: {field, y_label, scale_factor, short_id}
metrics = {
...
for m = 1:size(metrics,1)
    ...
    semMat  = nan(4,1);
    paired_stats = {};   % for within-genotype stats

    % ---------- (1) within-genotype paired tests ----------
    ...
    % ---------- (2) mixed-effects ANOVA: cond * geno ----------
    Ti2 = Ti;
    ...
    try
        ...
        rowCond = find(strcmp(a.Term,'cond'),1);
        rowGen  = find(strcmp(a.Term,'geno'),1);
        rowInt  = find(strcmp(a.Term,'cond:geno'),1);
    ...
    % ---------- (3) per-condition WT vs APP comparison ----------
    conds = ["baseline","ambtemp"];
    geno_rows = {};

    p_base = NaN; d_base = NaN;
    p_amb  = NaN; d_amb  = NaN;

    for ci = 1:numel(conds)
        c = conds(ci);
        Tc = Ti(Ti.cond==c, :);
        valsWT  = Tc.Y(Tc.geno=="WT");
        valsAPP = Tc.Y(Tc.geno=="APP");
        ...
        if nWT>=minN && nAPP>=minN
            ...
            d = local_cohen_d(valsWT, valsAPP);
        else
            p_t = NaN; d = NaN;
        end
    ...
    geno_tbl = cell2table(geno_rows, ...
        'VariableNames', {'cond','nWT','nAPP', ...
                          'MeanWT','MeanAPP', ...
                          'p_WTvsAPP','Cohen_d'});

    % ---------- build stats tables for output ----------
    ...
    OUT.metric_stats.(short_id).geno_cond = geno_tbl;

    % ---------- plotting ----------
    COL_WT  = [0.6 0.6 0.6];
    COL_APP = [0.39 0.58 0.93];

    figure('Color','w','Position',[200 200 600 430]); hold on;
    x = 1:4;
    ...
    % ---------- stars for within-genotype paired tests (raw p for now) ----------
    for gi = 1:numel(genotypes)
        gtype = genotypes(gi);
    ...
    text(2.5, ymax*0.63, txt, ...
        'HorizontalAlignment','center','VerticalAlignment','top', ...
        'FontSize',9);

    % NEW: per-condition WT vs APP text near bottom
    txt2 = sprintf('Base WT vs APP: p=%.3f, d=%.2f | Amb WT vs APP: p=%.3f, d=%.2f', ...
                   p_base, d_base, p_amb, d_amb);
    text(2.5, ymax*0.10, txt2, ...
        'HorizontalAlignment','center','VerticalAlignment','bottom', ...
        'FontSize',9);
    ...
    writetable(geno_tbl, ...
        fullfile(out_dir, sprintf('REM3h_%s_geno_cond_stats.csv', short_id)));
end
...
function d = local_cohen_d(x1, x2)
x1 = x1(~isnan(x1));
x2 = x2(~isnan(x2));
