function [ALL_REM, ALL_SUMMARY] = run_REM_MA_pipeline_ambtemp_cropped( ...
                                        data_dir, scores_dir, out_dir)
% run_REM_MA_pipeline_ambtemp_cropped
% -------------------------------------------------------------------------
% Like your original run_REM_MA_pipeline.m, but:
%   - only BASELINE + AMBTEMP
%   - uses CROPPED scores files: *_scored_scores_1Hz_crop.csv
%   - REM metrics are thus restricted to the 3 h window
%
% INPUTS
...
%
% OUTPUTS
%   ALL_REM      : all REM bouts across mice/conds (3 h windows only)
%   ALL_SUMMARY  : per-recording REM summary (3 h windows only)
% -------------------------------------------------------------------------

if nargin < 3 || isempty(out_dir)
    out_dir = fullfile(scores_dir,'REM_ambtemp_3h_out');
end
...
for iFile = 1:numel(files)
    ...
    if ~(cond=="baseline" || cond=="ambtemp")
        % only baseline + ambtemp in this pipeline
        continue;
    end
    ...
    fprintf('Processing %s with %s (3 h window)...\n', eeg_fname, scores_file);
...
save(fullfile(out_dir, 'REM_size_edges_3h.mat'), 'rem_size_edges_s');

fprintf('\n✓ REM+MA pipeline (3 h baseline + ambtemp) finished.\n');
