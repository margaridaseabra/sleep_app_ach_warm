function [states, epochs_t, MA_tbl, mouse_id, geno, cond] = read_scores_csv(scores_file)
% read_scores_csv
% -------------------------------------------------------------
...
%   20251001-baseline-mouse1-APP_scored_scores_1Hz.csv
    ...
    tokens = regexp(fname, ...
        '^(?<date>\d{8})[-_](?<cond>[^-_]+)[-_](?<mouse>[^-_]+)[-_](?<geno>[^-_.]+)_scored_scores_1Hz$', ...
        'names');

    if isempty(tokens)
        error(['Unexpected scores filename format: %s\n' ...
               'Expected something like: 20251001_baseline_mouse1_APP_scored_scores_1Hz'], ...
               fname);
    end
