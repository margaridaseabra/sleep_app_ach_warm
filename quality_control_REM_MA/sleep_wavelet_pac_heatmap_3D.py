...
from matplotlib.widgets import SpanSelector, Slider  # ADD Slider here
...
PAC3D_PHASE_CENTERS = np.arange(4.0, 12.1, 1.0)   # Hz (y-axis)
PAC3D_AMP_CENTERS   = np.arange(30.0, 80.1, 5.0)  # Hz (x-axis)
PAC3D_PHASE_BW = 2.0   # Hz
PAC3D_AMP_BW   = 10.0  # Hz

PAC3D_WIN_SEC  = 4.0
PAC3D_STEP_SEC = 1.0

PAC3D_DOWNSAMPLE_FS = 250.0  # None disables; must keep Nyquist > amp_hi
...
def pac_mvl_from_phase_amp(phase, amp, normalize=True):
    """MVL PAC. normalize=True makes it scale-free (recommended)."""
    z = amp * np.exp(1j * phase)
...
def _moving_mean(x, win_n):
    """Fast moving mean using cumulative sum. Works for real/complex."""
    x = np.asarray(x)
    c = np.cumsum(np.concatenate([[0], x]))
    return (c[win_n:] - c[:-win_n]) / win_n  # length = n-win_n+1
...
def pac_cube_mvl(
    ...
    Returns:
      ...
      phase_centers : (n_p,)
      amp_centers : (n_a,)
      cube : (n_p, n_a, n_t)  [normalized MVL]
    """
    # optional resample
    x, fs2 = _resample_for_pac(eeg_win, fs, downsample_fs)
    ...
    for pi, fp in enumerate(phase_centers):
        ...
        # for each amp band: moving mean of amp*e (complex) -> abs -> normalize
        for ai in range(len(amp_centers)):
            z = amp_envs[ai] * e
            mean_z = _moving_mean(z, win_n)  # (valid_len,)
            mean_z = mean_z[i0s]             # (n_t,)
...
class BoutViewer:
    def __init__(self, base, eeg, emg, fs, time_s, states, bouts_idx, state_label, ach=None, fs_ach=None):
        ...
        self.last_span = None  # store last dragged window
    ...
    def open_pac3d(self):
        ...
        # Use last dragged span if available; otherwise use full current window
        if self.last_span is not None:
            t0, t1 = self.last_span
        else:
            # fall back to currently displayed bout-window
            start_idx, end_idx = self.bouts_idx[self.idx]
            start_s = float(self.time_s[start_idx])
            end_s = float(self.time_s[end_idx - 1] + 1.0)
...
class PAC3DExplorer:
    ...
    def _draw(self):
        ...
        if self.cube.shape[2] == 0:
            self.ax_map.text(0.5, 0.5, "Window too short for PAC cube", ha="center", va="center",
                             transform=self.ax_map.transAxes)
            self.fig.canvas.draw_idle()
            return

        C = self.cube[:, :, self.ti]  # (phase, amp)
        ...
        self.ax_map.set_xlabel("Amplitude (Hz)")
        self.ax_map.set_ylabel("Phase (Hz)")
        self.ax_map.set_title(f"Comodulogram @ t={self.t[self.ti]:.1f}s (norm MVL)")

        # crosshair for selected cell
        self.ax_map.axvline(self.amp_c[self.sel_ai], linestyle="--", alpha=0.8)
        self.ax_map.axhline(self.phase_c[self.sel_pi], linestyle="--", alpha=0.8)

        # time series at selected (phase,amp)
        y = self.cube[self.sel_pi, self.sel_ai, :]
        self.ax_ts.plot(self.t, y)
        self.ax_ts.set_xlabel("Time (s)")
        self.ax_ts.set_ylabel("PAC (norm MVL)")
        self.ax_ts.set_title(f"PAC(t) at phase={self.phase_c[self.sel_pi]:.1f} Hz, amp={self.amp_c[self.sel_ai]:.1f} Hz")

        # colorbar (recreate each draw for simplicity)
        # (If you want: optimize by keeping one cbar handle.)
        self.fig.colorbar(self.im, ax=self.ax_map, fraction=0.046, pad=0.04, label="PAC (norm MVL)")
