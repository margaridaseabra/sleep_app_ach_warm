...
class BoutViewer:
    ...
    def update_plot(self):
        ...
        # ===========================
        # PAC(t) panel (MVL, sliding window)
        # ===========================
        if PAC_ENABLED:
            try:
                ...
                # Optional horizontal surrogate threshold
                if PAC_SURROGATES_N and PAC_SURROGATES_N > 0:
                    thr = pac_surrogate_threshold_blockswap(
                        phase, amp, fs,
                        n_surr=PAC_SURROGATES_N,
                        block_sec=PAC_SURR_BLOCK_SEC,
                        pct=PAC_SURR_PCT,
                        seed=PAC_SURR_SEED
                    )
                    if np.isfinite(thr):
                        ax_pac.axhline(thr, linestyle="--", color="red", alpha=0.7,
                                       label=f"{PAC_SURR_PCT:.0f}th % surrogate")
                        ax_pac.legend(fontsize=8, loc="upper right")
