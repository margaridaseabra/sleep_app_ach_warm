...
# ===========================
# Wavelet cycles "config knob"
# ===========================
# In THIS Morlet-like implementation:
#   wavelet(u) = exp(i*2*pi*f0*u) * exp(-u^2/2)
# with u = t/scale.
# The Gaussian envelope is ~significant over roughly ±3σ -> width ~6 in u-units,
# so the number of oscillatory cycles under the envelope is approximately:
#   cycles ≈ 6 * f0
#
# Therefore:
#   f0 ≈ cycles / 6
#
# Examples:
#   WAVELET_CYCLES = 6  -> f0 = 1.0  (your original behavior)
#   WAVELET_CYCLES = 3  -> f0 = 0.5  (more time resolution, less freq resolution)
WAVELET_CYCLES = 6.0
WAVELET_F0 = float(WAVELET_CYCLES) / 6.0
...
def pac_mvl_heatmap(eeg_win, fs, win_start_sec,
    ...
    n_b = len(amp_bands)
    pac_map = np.zeros((n_b, len(i0s)), dtype=float)
...
class BoutViewer:
    ...
    def open_detail_wavelet(self, t0, t1):
        ...
        fig.suptitle(
            f"Detail wavelet {band_label}\n{t0:.2f}–{t1:.2f} s | ~{WAVELET_CYCLES:g} cycles",
            fontsize=12,
            fontweight='bold'
        )
    ...
    def update_plot(self):
        ...
        # Panel 4: Wavelet scalogram (uses WAVELET_F0)
        ...
        freqs_wv, scalogram_db = compute_wavelet_scalogram_matlab_like(
            ...
            f0=WAVELET_F0, exp_corr=0.0, time_smooth=0.0, baseline_mode="window",
        )
        ...
        if PAC_ENABLED:
            try:
                if PAC_MODE == "heatmap":
                    t_edges_pac, y_edges, pac_map = pac_mvl_heatmap(
        ...
        self.fig.suptitle(
            ...
            f"Navigation: ←/→ or n/p | Quit: q/Esc | Detail wavelet: drag on EEG | 3D PAC: d key | "
            f"Wavelet ~{WAVELET_CYCLES:g} cycles\n"
...
def main():
    ...
    for mat_path in mat_files:
        ...
        print(f"  Wavelet cycles knob: ~{WAVELET_CYCLES:g} cycles (f0={WAVELET_F0:g})")
