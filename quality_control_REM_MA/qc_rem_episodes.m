...
% A MAT file '<eegBase>_REM_QC.mat' is also saved next to eeg_mat_file.
    ...
    addParameter(p,'fmax',30,@(x)isscalar(x)&&x>0);  % max freq for Morlet
    ...
    while i <= nREM
        ...
        switch choice
            ...
            otherwise
                % unknown key: re-show same episode
        end
    end
...
% =====================================================================
% Helper: episode viewer (EEG, EMG, Morlet, PSD + metrics)
% =====================================================================
function fig = plot_one_episode(t, eeg, emg, state_win, ...
    ...
    % --- 4) Morlet time–frequency (0–15 Hz), z-scored per freq ----------
    ax4 = subplot(5,1,4);
    try
        [wt, f_wt] = cwt(eeg, fs, 'amor', 'FrequencyLimits',[0.5 15]);
        power = abs(wt).^2;
    ...
    % --- 5) PSD inside episode core + metrics ---------------------------
    ax5 = subplot(5,1,5);
    ...
    % nested callback to record key (including arrows)
