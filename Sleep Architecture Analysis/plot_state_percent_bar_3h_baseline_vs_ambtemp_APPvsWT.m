function OUT = plot_state_percent_bar_3h_baseline_vs_ambtemp_APPvsWT(rows_overall_ambtemp, out_dir, varargin)
% plot_state_percent_bar_3h_baseline_vs_ambtemp_APPvsWT
% -------------------------------------------------------------------------
% Bar plot comparing 3 h BASELINE vs 3 h ambtemp, separately in WT vs APP.
%
...
%   2) WT ambtemp    (grey outline)
%   3) APP baseline  (solid blue)
%   4) APP ambtemp   (blue outline)
%
% Stats:
...
if isempty(T)
    warning('No baseline + ambtemp rows found. Nothing to plot.');
...
for si = 1:nStates
    ...
    if height(Twide_all) >= 2 && nWT_all > 0 && nAPP_all > 0
        try
            ...
            L.Condition = categorical(L.Condition, {'baseline','ambtemp'}); % enforce order
...
% --- stars for paired baseline vs ambtemp within genotype ---
for si = 1:nStates
    st = state_order(si);
    for gi = 1:nGen
        gtype = genotypes(gi);
...
title('3 h baseline vs 3 h ambtemp – WT vs APP');
...
fig_file = fullfile(out_dir, 'state_percent_3h_baseline_vs_ambtemp_APPvsWT_bar.png');
saveas(gcf, fig_file);
