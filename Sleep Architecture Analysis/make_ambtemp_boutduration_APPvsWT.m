function OUT = make_ambtemp_boutduration_APPvsWT(PERHOUR, out_dir, states_to_use, varargin)
% make_ambtemp_boutduration_APPvsWT
% -------------------------------------------------------------------------
% ambtemp-only plot of:
...
%
% Additionally:
%   - Exports a CSV with per-mouse mean bout duration in a
%     Prism-friendly "Grouped" layout:
%         row1: group names (WT ... WT  APP ... APP)
%         row2: mouse IDs (or generic IDs)
%         rows: states (WK, MA, NREM, REM, ...)
%
% INPUT
%   PERHOUR       : OUT.per_hour table from run_group_sleep_architecture
%   out_dir       : folder to save the figure/CSV (default: pwd)
...
%   OUT.csv_file     : CSV filename with per-mouse data (Prism-style)
% -------------------------------------------------------------------------
...
is_ambtemp = cond_str == "ambtemp";
PH = PH(is_ambtemp, :);

if isempty(PH)
    warning('No ambtemp rows in PERHOUR. Nothing to plot.');
    OUT = struct('success', false, 'msg', 'no ambtemp data');
    return;
end
...
if ~all(ismember(needed, PH.Properties.VariableNames))
    error('PERHOUR table missing required columns for make_ambtemp_boutduration_APPvsWT.');
end
...
if isempty(state_order)
    warning('None of the requested states found in PERHOUR ambtemp data.');
...
title('ambtemp: mean bout duration per state (WT vs APP)');
...
if any(stats_idx)
    ...
    fprintf('\nambtemp mean bout duration: WT vs APP stats (per state)\n');
    disp(stats_tbl);
else
    stats_tbl = table();
...
fname_fig = sprintf('ambtemp_boutduration_APPvsWT%s.png', id_suffix);
out_file  = fullfile(out_dir, fname_fig);
saveas(gcf, out_file);

% ---------- 10) NEW: Export per-mouse data to CSV (Prism-style grouped) ----------
try
    % unique mice per genotype (sorted)
    WT_mice  = unique(Tmouse.mouse(Tmouse.geno=="WT"));
    APP_mice = unique(Tmouse.mouse(Tmouse.geno=="APP"));
    nWT_all  = numel(WT_mice);
    nAPP_all = numel(APP_mice);

    nRowsData = numel(state_order);
    nCols     = 1 + nWT_all + nAPP_all; % 1 for "State" column

    % numeric matrices (rows = states, cols = mice)
    dataWT  = NaN(nRowsData, nWT_all);
    dataAPP = NaN(nRowsData, nAPP_all);
    ...
    for ii = 1:nRowsData
        ...
        for jj = 1:nWT_all
            m_id = WT_mice(jj);
            idx  = (Ti.geno == "WT") & (Ti.mouse == m_id);
            if any(idx)
                dataWT(ii,jj) = Ti.mean_bout_dur_s(find(idx,1,'first'));
            end
        end
        ...
        for jj = 1:nAPP_all
            m_id = APP_mice(jj);
            idx  = (Ti.geno == "APP") & (Ti.mouse == m_id);
            if any(idx)
                dataAPP(ii,jj) = Ti.mean_bout_dur_s(find(idx,1,'first'));
            end
        end
    end

    % Build cell array with 2 header rows + data
    nHeaderRows = 2;
    C = cell(nHeaderRows + nRowsData, nCols);
    ...
    if nAPP_all > 0
        C(1, 2+nWT_all : 1+nWT_all+nAPP_all) = {'APP'};
    end

    % Row 2: mouse IDs (or generic IDs)
    C{2,1} = 'State';
    if nWT_all > 0
        C(2, 2:1+nWT_all) = cellstr(WT_mice(:)).';
    end
    if nAPP_all > 0
        C(2, 2+nWT_all : 1+nWT_all+nAPP_all) = cellstr(APP_mice(:)).';
    end

    % Data rows
    for ii = 1:nRowsData
        row_idx = nHeaderRows + ii;
        C{row_idx,1} = char(state_order(ii));
        if nWT_all > 0
            C(row_idx, 2:1+nWT_all) = num2cell(dataWT(ii,:));
        end
        if nAPP_all > 0
            C(row_idx, 2+nWT_all : 1+nWT_all+nAPP_all) = num2cell(dataAPP(ii,:));
        end
    end

    % Write CSV
    fname_csv = 'ambtemp_boutduration_APPvsWT_forPrism.csv';
    csv_file  = fullfile(out_dir, fname_csv);
    writecell(C, csv_file);
    fprintf('✅ CSV (per-mouse, Prism-style) saved to: %s\n', csv_file);
catch ME
    % Safer: use an ID and a format specifier so '%' in the message
    % cannot be interpreted as formatting
    warning('CSV:WritePrismStyle', ...
        'Could not write Prism-style CSV (%s): %s', ...
...
fprintf('✅ ambtemp bout duration state plot saved to: %s\n', out_file);
end
