function OUT = make_baseline_total_bouts_per_hour_APPvsWT_2_prism(rows_perhr, out_dir, states_to_plot, varargin)
...
%
% PRISM EXPORT (added):
%   For each state, also write a CSV:
%       baseline_bouts_per_hour_<state>_forPrism.csv
%
%   Layout:
%       Hour, GENO_mouseID, GENO_mouseID2, ...
%       0,    3.2,          2.8,          ...
%       1,    ...
%       ...
%
...
%
%   PRISM:
%   OUT.prism.(STATE).table  : Prism-style table (rows = hours, cols = mice)
%   OUT.prism.(STATE).csv    : path to CSV file
% -------------------------------------------------------------------------
...
OUT.varstats       = struct();
OUT.anova          = struct();
OUT.prism          = struct();   % <--- NEW: container for Prism exports
...
for s = 1:numel(states_to_plot)
    ...
    OUT.anova.(matlab.lang.makeValidName(st)).rm       = rm;
    OUT.anova.(matlab.lang.makeValidName(st)).within   = withinTbl;
    OUT.anova.(matlab.lang.makeValidName(st)).between  = betweenTbl;
    ...
    OUT.var_files.(matlab.lang.makeValidName(st)) = out_file_var;
    ...
    OUT.varstats.(matlab.lang.makeValidName(st)) = VAR;

    % ---------------------------------------------------------------------
    % 9) PRISM EXPORT FOR THIS STATE
    %     Rows = hours, Columns = mice (with genotype in name)
    % ---------------------------------------------------------------------
    % dataMat : nM (mice) × nH (hours)  -> we want nH × nM
    prismMat = dataMat.';                 % nH × nM
    hourCol  = all_hours(:);              % nH × 1

    % Build column names like WT_mouse12, APP_mouse03, etc.
    colNames_raw = cell(1, nM);
    for i = 1:nM
        colNames_raw{i} = sprintf('%s_%s', char(genoVec(i)), char(mice(i)));
    end
    % Make them valid MATLAB variable names (Prism doesn't care, but writetable does)
    ...
    Tprism = addvars(Tprism, hourCol, 'Before', 1, 'NewVariableNames', 'Hour');

    csvName = sprintf('baseline_bouts_per_hour_%s_forPrism.csv', lower(st));
    csvPath = fullfile(out_dir, csvName);
    try
        writetable(Tprism, csvPath);
        fprintf('📄 Prism CSV for state %s saved to: %s\n', st, csvPath);
    catch ME
        warning('make_baseline_total_bouts_per_hour_APPvsWT_2:PrismWriteFailed', ...
                'Could not write Prism CSV for %s (%s): %s', st, csvPath, ME.message);
        csvPath = '';
    end

    stField = matlab.lang.makeValidName(st);
    OUT.prism.(stField).table = Tprism;
    OUT.prism.(stField).csv   = csvPath;
