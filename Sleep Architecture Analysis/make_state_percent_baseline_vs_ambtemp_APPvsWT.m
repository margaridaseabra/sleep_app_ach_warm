function OUT = make_state_percent_baseline_vs_ambtemp_APPvsWT(rows_overall, out_dir, varargin)
% make_state_percent_baseline_vs_ambtemp_APPvsWT
%- -------------------------------------------------------------------------
% Compare 3 h BASELINE vs 3 h AMBTEMP, keeping WT vs APP:
%
%   Y: % of 3 h window spent in each state
%   Factors:
%       - Condition: baseline vs ambtemp  (within mouse, paired)
%       - Genotype : WT vs APP           (between mice)
%
% For each state (WK, MA, NREM, REM, SLEEP):
%   - Compute per-mouse %time in state for baseline and ambtemp
%   - Paired t-test (baseline vs ambtemp) separately for WT and APP
%   - Compare change Δ = ambtemp - baseline between WT and APP (unpaired t-test)
%
% Plot:
%   - 2 panels: top WT, bottom APP
%   - x-axis: states
%   - per mouse: line from baseline to ambtemp
...
%                  (but run on cropped 3h baseline + 3h ambtemp files)
...
keep_cond = cond_str == "baseline" | cond_str == "ambtemp";
T = T(keep_cond, :);

if isempty(T)
    warning('No baseline/ambtemp rows found in rows_overall. Nothing to plot.');
...
% ---------- 3) Within-genotype paired stats (baseline vs ambtemp) ----------
genotypes = ["WT","APP"];
...
for gi = 1:nGen
    ...
    for si = 1:nStates
        ...
        % pivot: one row per mouse, columns: baseline, ambtemp
        ...
        [~, p_pair] = ttest(x, y);    % baseline vs ambtemp
        delta = y - x;
...
for gi = 1:nGen
    ...
    title(sprintf('%s: 3 h baseline vs 3 h ambtemp', gtype));
    x = 1:nStates;
...
fig_file = fullfile(out_dir, 'state_percent_3h_baseline_vs_ambtemp_APPvsWT.png');
saveas(gcf, fig_file);
...
fprintf('✅ 3h baseline vs ambtemp state-percent figure saved to: %s\n', fig_file);
end
