function OUT = make_ambtemp_state_percent_APPvsWT_prism(rows_overall, out_dir, split_by_sex, varargin)
% make_ambtemp_state_percent_APPvsWT_prism
% ---------------------------------------------------------------
...
% ---- 1) Keep only ambtemp recordings ----
cond_str    = lower(strtrim(T.condition));
is_ambtemp = cond_str == "ambtemp";
T = T(is_ambtemp, :);

if isempty(T)
    warning('No ambtemp rows found in rows_overall (condition=="ambtemp"). Nothing to plot.');
...
if isempty(base_states)
    warning('No WK/MA/NREM/REM states found in ambtemp rows.');
...
prism_long_csv = fullfile(out_dir, 'ambtemp_state_percent_APPvsWT_PrismLong.csv');
try
    writetable(prism_long_tbl, prism_long_csv);
catch ME
    warning('make_ambtemp_state_percent_APPvsWT_prism:PrismLongWriteFailed', ...
...
if ~split_by_sex
    ...
    if ~hasWT && ~hasAPP
        warning('No WT or APP genotypes found in ambtemp data.');
    ...
    if any(stats_idx)
        ...
        fprintf('\nambtemp %%time in state: WT vs APP stats (per state)\n');
        disp(stats_tbl);
    ...
    prism_summary_csv = fullfile(out_dir, 'ambtemp_state_percent_APPvsWT_PrismSummary.csv');
    try
        writetable(prism_summary_tbl, prism_summary_csv);
    catch ME
        warning('make_ambtemp_state_percent_APPvsWT_prism:PrismSummaryWriteFailed', ...
                'Could not write Prism summary CSV (%s): %s', prism_summary_csv, ME.message);
        prism_summary_csv = '';
    end

    ttl     = 'ambtemp: % time in each state (WT vs APP, incl. SLEEP)';
    out_png = 'ambtemp_state_percent_APPvsWT.png';
    ...
    if ~isempty(rows_state)
        prism_summary_tbl = table( ...
            ...
            'VariableNames', {'State','GroupLabel','Genotype','Sex', ...
                              'Mean_Pct','SEM_Pct'});
        prism_summary_csv = fullfile(out_dir, 'ambtemp_state_percent_APPvsWT_bySex_PrismSummary.csv');
        try
            writetable(prism_summary_tbl, prism_summary_csv);
        catch ME
            warning('make_ambtemp_state_percent_APPvsWT_prism:PrismSummaryBySexWriteFailed', ...
    ...
    ttl     = 'ambtemp: % time in each state (WT vs APP, by sex, incl. SLEEP)';
    out_png = 'ambtemp_state_percent_APPvsWT_bySex.png';
end
...
fprintf('✅ ambtemp %%time state plot saved to: %s\n', out_file);
