function OUT = make_ambtemp_total_bouts_per_hour_APPvsWT_2(rows_perhr, out_dir, states_to_plot, varargin)
% make_ambtemp_total_bouts_per_hour_APPvsWT_2
% -------------------------------------------------------------------------
% For AMBTEMP recordings only (condition=="ambtemp"):
...
%       ambtemp_bouts_per_hour_<state>_APPvsWT_hour_by_mouse.csv
%
% INPUT
...
is_ambtemp  = cond_str == "ambtemp";   % change this if your label differs
P = P(is_ambtemp, :);

if isempty(P)
    warning('No ambtemp rows found in rows_perhr (condition=="ambtemp"). Nothing to plot.');
...
for s = 1:numel(states_to_plot)
    ...
    Pst = P(P.state == st, :);
    if isempty(Pst)
        warning('No ambtemp rows for state "%s". Skipping.', st);
        continue;
    end
    ...
    csv_name_hour = sprintf('ambtemp_bouts_per_hour_%s_APPvsWT_hour_by_mouse.csv', lower(st));
    ...
    if numel(uniqG) < 2
        ...
        warning('State %s (ambtemp): only one genotype present for RM ANOVA. Skipping fitrm.', st);
        rm         = [];
        withinTbl  = table();
        betweenTbl = table();
    else
    ...
    fprintf('\n===== 2-way RM ANOVA (Time x Genotype) for %s bouts/hour (ambtemp) =====\n', st);
    ...
    title(sprintf('Ambtemp: bouts per hour in %s (WT vs APP)', st));
    ...
    fname = sprintf('ambtemp_bouts_per_hour_%s_APPvsWT%s%s.png', ...
    ...
    fprintf('✅ Ambtemp bouts/hour time-course plot for %s saved to: %s\n', st, out_file);
    ...
    if any(stats_idx)
        ...
        fprintf('\nAmbtemp bouts/hour (%s): WT vs APP per-hour stats (exploratory)\n', st);
        disp(stats_tbl);
    ...
    % For each mouse: mean bouts/hour across *all ambtemp hours* for this state
    mean_mouse = nan(nM,1);
    for i = 1:nM
        row = dataMat(i,:);
        row = row(~isnan(row));
    ...
    title(sprintf('Ambtemp: between-mice variability of %s bouts/hour', st));
    set(gca,'Box','off');
    ...
    fname_var = sprintf('ambtemp_bouts_per_hour_%s_APPvsWT_betweenmice%s.png', ...
                        lower(st), id_suffix2);
