function OUT = plot_bouts_per_hour_ambtemp_progression_APPvsWT( ...
                rows_perhr, meta_tbl, states_to_plot, out_dir, varargin)
% plot_bouts_per_hour_ambtemp_progression_APPvsWT
% -------------------------------------------------------------------------
...
%   'ambWindow_h'         : duration of the manipulation (for shade), default 3
...
% keep only ambtemp recordings
P = P(P.condition=="ambtemp",:);
if isempty(P)
    warning('No ambtemp rows in rows_perhr.');
    return;
end
...
for s = 1:numel(states_to_plot)
    ...
    Jst = J(J.state==st,:);
    if isempty(Jst)
        warning('No ambtemp rows for state %s in progression; skipping.', st);
        continue;
    end
    ...
    % vertical shading for ambtemp window [0, ambWindow]
    yl = ylim;
    ...
    title(sprintf('Ambtemp progression: %s bouts/hour (WT vs APP)', st));
    ...
    fname = sprintf('ambtemp_progression_bouts_per_hour_%s_APPvsWT.png', lower(st));
...
OUT.success = true;
fprintf('✅ Ambtemp progression plots + stats saved in %s\n', out_dir);
