function rows_overall_drugs = run_group_sleep_architecture_drugs_overall(input_dir, varargin)
% run_group_sleep_architecture_drugs_overall
% -------------------------------------------------------------------------
% Minimal version for drugs project:
...
%
%   - Returns this as rows_overall_drugs
%   - Saves:
%       rows_overall_drugs.mat
%       rows_overall_drugs.csv
%
% No per-hour tables, no ANOVA, no plots.
%
% Assumes filenames like:
%   20251005_baseline_mouse8_WT_scored_scores_1Hz_crop.csv
%   20251014_drugs_mouse11_WT_scored_scores_1Hz_crop.csv
%
% i.e. pattern:
...
%   'out_dir' : folder to save group files (default: input_dir/group_overall_drugs)
% -------------------------------------------------------------------------
...
if isempty(S.out_dir)
    S.out_dir = fullfile(S.input_dir, 'group_overall_drugs');
end
...
% ----------------- save & return -----------------
rows_overall_drugs = rows_overall;

mat_file  = fullfile(S.out_dir, 'rows_overall_drugs.mat');
csv_file  = fullfile(S.out_dir, 'rows_overall_drugs.csv');

save(mat_file, 'rows_overall_drugs');
if ~isempty(rows_overall_drugs)
    writetable(rows_overall_drugs, csv_file);
else
    warning('rows_overall_drugs is empty; not writing CSV.');
end

fprintf('\n✅ rows_overall_drugs created with %d rows.\n', height(rows_overall_drugs));
fprintf('   MAT: %s\n', mat_file);
fprintf('   CSV: %s\n', csv_file);
end
