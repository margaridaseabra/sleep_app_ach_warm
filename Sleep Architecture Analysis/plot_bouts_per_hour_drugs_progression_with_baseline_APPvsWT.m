function OUT = plot_bouts_per_hour_drugs_progression_with_baseline_APPvsWT( ...
                rows_perhr, meta_tbl, states_to_plot, out_dir, varargin)
% plot_bouts_per_hour_drugs_progression_with_baseline_APPvsWT
% -------------------------------------------------------------------------
% Progression of bouts/hour around drugs start, WT vs APP,
% with BASELINE overlay and per-bin (per hour) 2x2 ANOVA:
%   Condition (baseline vs drugs) x Genotype (WT vs APP)
% using mixed-effects:
%   bouts_per_h ~ condition * genotype + (1|mouse)
%
% Stars on the plot:
%   black *    : main effect of condition (baseline vs drugs), p<0.05
...
%                  'Time started (s)'  (drugs onset, seconds from rec start)
%
...
%   'drugWindow_h'         : duration of manipulation in hours (for shaded patch, default 3)
...
P = P(P.condition=="baseline" | P.condition=="drugs",:);
if isempty(P)
    warning('No baseline or drugs rows in rows_perhr.');
    return;
end

P_amb  = P(P.condition=="drugs",:);
P_base = P(P.condition=="baseline",:); %#ok<NASGU> % (not used directly but kept for clarity)

if isempty(P_amb)
    warning('No drugs rows; cannot anchor relative hours.');
    return;
end
...
% ---------- 4) Join drugs with meta & find anchor hour h0 per mouse ----------
J_amb = innerjoin(P_amb, meta_small, 'Keys','mouse_id');

if isempty(J_amb)
    warning('No overlap between drugs rows and meta_tbl (by numeric mouse_id).');
    return;
end
...
% For each mouse, anchor hour_idx where rel_hour_bin == 0 (drugs start)
...
% ---------- 5) Attach h0 to BOTH baseline & drugs, compute rel_hour_bin ----------
...
for s = 1:numel(states_to_plot)
    ...
    % -------- 6a) Per-bin means / SEMs for baseline and drugs ----------
    ...
    for b = 1:nBins
        ...
        haveAmb  = any(Kb.condition=="drugs");
        ...
        Kb2.condition = categorical(Kb2.condition, {'baseline','drugs'});
    ...
    % Shaded drugs window [0, drugWindow]
    yl = ylim;
    ...
    title(sprintf('drugs vs baseline: %s bouts/hour (WT vs APP)', st));
    ...
    star_txt = '* black: Condition (baseline vs drugs) p<0.05;   ' + ...
    ...
    fname = sprintf('drugs_progression_withBaseline_%s_APPvsWT.png', lower(st));
    ...
    % also write CSV
    csv_out = fullfile(out_dir, sprintf('drugs_progression_perbin_ANOVA_%s.csv', lower(st)));
...
OUT.success = true;
fprintf('✅ drugs progression with baseline overlay + per-bin ANOVA saved in %s\n', out_dir);
