function OUT = run_baseline_boutduration_threePhase_APPvsWT_prism(PERHOUR, out_dir, states_to_use, varargin)
% run_baseline_boutduration_threePhase_APPvsWT_prism
% -------------------------------------------------------------------------
% Baseline-only analysis of mean bout duration per state, in THREE windows:
%
%   Phase 1 (0–3 h):  hour_idx in [0 1 2]
%   Phase 2 (3–6 h):  hour_idx in [3 4 5]
%   Washout:          all remaining baseline hours
%
...
%   baseline_boutdur_threePhase_<state>_forPrism.csv
%
%   Layout:
...
if ~all(ismember(needed, PH.Properties.VariableNames))
    error('PERHOUR table missing required columns for run_baseline_boutduration_threePhase_APPvsWT_prism.');
end
...
% ---------- Overall bout duration across entire baseline (no phases) ----------
...
overall_csvPath = fullfile(out_dir, 'baseline_boutdur_wholeRecording_forPrism.csv');
try
    writetable(T_overallPrism, overall_csvPath);
    fprintf('📄 Prism CSV for WHOLE recording bout duration saved to: %s\n', overall_csvPath);
catch ME
    warning('run_baseline_boutduration_threePhase_APPvsWT_prism:OverallPrismWriteFailed', ...
...
phaseHours   = {0:2, 2:5, []};     % washout will be filled per state
...
for s = 1:numel(states_to_use)
    ...
    washH   = setdiff(h_all, base12);   % washout = any other baseline hours
    phaseHours{3} = washH;
    ...
    title(sprintf('Baseline: bout duration (%s), three phases, WT vs APP', st_upper));
    set(gca,'Box','off');
    ...
    fname    = sprintf('baseline_boutdur_threePhase_%s_APPvsWT%s.png', lower(st_upper), id_suffix);
    ...
    csvName = sprintf('baseline_boutdur_threePhase_%s_forPrism.csv', lower(st_upper));
    csvPath = fullfile(out_dir, csvName);
    try
        writetable(Tprism, csvPath);
        fprintf('📄 Prism CSV for bout duration (%s) saved to: %s\n', st_upper, csvPath);
    catch ME
        warning('run_baseline_boutduration_threePhase_APPvsWT_prism:PrismWriteFailed', ...
