function OUT = plot_wake_after_REM_by_type(REM_TRANS, out_dir)
% Baseline only.
% Y: wake_after_rem_dur_s (converted to minutes)
% X: REM type (Short/Medium/Long)
% Bars WT vs APP, with mouse dots + IDs.
...
if isempty(T)
    warning('No baseline REM bouts.'); OUT.success=false; return;
end

T.wake_after_min = T.wake_after_rem_dur_s / 60;

G = groupsummary(T, {'mouse','genotype','rem_type'}, 'mean', 'wake_after_min');
G.Properties.VariableNames{end} = 'mean_wake_after_min';
...
if ~hasWT && ~hasAPP
    warning('No WT/APP found.'); OUT.success=false; return;
end
...
for i = 1:nTypes
    ...
    if hasAPP
        ...
        vAPP = G.mean_wake_after_min(mAPP);
...
for i = 1:nTypes
    ...
    if hasWT
        mWT = (G.genotype=="WT") & (G.rem_type==rt);
        v = G.mean_wake_after_min(mWT); mids = G.mouse(mWT);
    ...
    if hasAPP
        mAPP = (G.genotype=="APP") & (G.rem_type==rt);
        v = G.mean_wake_after_min(mAPP); mids = G.mouse(mAPP);
...
xlabel('REM type');
ylabel('Wake duration after REM (min)');
title('Baseline: wake after REM (WT vs APP, by REM type)');
if hasWT && hasAPP
    legend({'WT','APP'},'Location','northoutside','Orientation','horizontal');
end
set(gca,'Box','off','FontSize',12);

out_file = fullfile(out_dir,'baseline_wakeAfter_REM_byType_APPvsWT.png');
saveas(gcf,out_file);

OUT = struct('success',true,'file',out_file,'rem_types',rem_types);
fprintf('✅ Saved wake-after-REM plot to %s\n', out_file);
end
