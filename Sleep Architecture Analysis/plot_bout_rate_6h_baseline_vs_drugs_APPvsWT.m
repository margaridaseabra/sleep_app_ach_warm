function OUT = plot_bout_rate_6h_baseline_vs_drugs_APPvsWT(rows_overall_drugs, out_dir, varargin)
% plot_bout_rate_6h_baseline_vs_drugs_APPvsWT
% -------------------------------------------------------------------------
% Uses the CROPPED 6 h baseline + 6 h drugs OVERALL table to compute:
%
...
%       WT drugs    (grey outline)
%       APP baseline  (solid blue)
%       APP drugs   (blue outline)
%
% Stats:
%   - Within-genotype paired t-test (baseline vs drugs), WT and APP
%       • BH-FDR across states
%       • stars drawn between the 2 bars (per genotype)
%   - Δ = drugs - baseline per mouse, WT vs APP (unpaired t-test)
%   - 2x2 mixed ANOVA (Condition × Genotype) via mixed-effects model
%       • prints C, G, G×C p-values above each state
%
% INPUT
%   rows_overall_drugs : table from run_group_sleep_architecture_drugs_overall
...
mask_cond = T.condition=="baseline" | T.condition=="drugs";
T = T(mask_cond,:);
if isempty(T)
    warning('No baseline + drugs rows in rows_overall_drugs.');
...
p_within = nan(nGen,nStates);    % paired baseline vs drugs per genotype
...
for si = 1:nStates
    ...
    if exist('Twide_all','var') && ~isempty(Twide_all) && nWT>0 && nAPP>0
        ...
        L.Condition = categorical(L.Condition, {'baseline','drugs'});
...
% stars for paired baseline vs drugs
for si = 1:nStates
    st = state_order(si);
    for gi = 1:nGen
        gt = genotypes(gi);
...
xlabel('State');
ylabel('Bouts per hour (6 h window)');
title('6 h baseline vs 6 h drugs – bouts/hour (WT vs APP)');
...
fig_file = fullfile(out_dir,'bout_rate_6h_baseline_vs_drugs_APPvsWT.png');
saveas(gcf, fig_file);

% save stats
if ~isempty(within_tbl)
    writetable(within_tbl, fullfile(out_dir,'bout_rate_6h_within_cond_stats.csv'));
end
if ~isempty(anova_tbl)
    writetable(anova_tbl, fullfile(out_dir,'bout_rate_6h_anova_stats.csv'));
end
...
fprintf('✅ 6 h bout-rate bar plot + stats saved in %s\n', out_dir);
end
