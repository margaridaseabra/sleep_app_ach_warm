...
%       .ranova     : ranova table (within: Condition)
%       .anova_between : anova table (between: Genotype)
%       .post_cond        : multcompare(rm,'Condition',...)
%       .post_cond_by_geno: multcompare(rm,'Condition','By','geno_group',...)
%       .post_geno_by_cond: multcompare(rm,'geno_group','By','Condition',...)
%       .Twide      : wide table (one row per mouse)
% -------------------------------------------------------------------------
...
if fid>0
    ...
    fprintf(fid, 'Conditions: baseline, ambtemp, drugs\n\n');
end
...
for s = 1:numel(presentStates)
    ...
    % convert to categorical genotype
    Tstate.geno_group = categorical(Tstate.geno_group);

    % unstack: columns = conditions (baseline/ambtemp/drugs)
    Twide = unstack(Tstate, 'pct_rec', 'condition');
    % Twide now has columns: mouse, geno_group, baseline, ambtemp, drugs (if present)

    % Keep only rows with genotype info
    Twide = Twide(~isundefined(Twide.geno_group), :);

    % Determine which repeated-measure columns exist and in which order
    rm_cond_cols = conds(ismember(conds, Twide.Properties.VariableNames));
    if numel(rm_cond_cols) < 2
        continue;
    end

    % Build formula like 'baseline-drugs ~ geno_group'
    respRange = sprintf('%s-%s', rm_cond_cols(1), rm_cond_cols(end));
    formula = sprintf('%s ~ geno_group', respRange);
    ...
    anovatbl_between = anova(rm);  %#ok<NASGU>

    % Post-hoc (Dunn–Sidak == Sidak)
    % 1) Overall condition main effect
    post_cond = multcompare(rm, 'Condition', ...
                            'ComparisonType', 'dunn-sidak');

    % 2) Condition comparisons within each genotype
    post_cond_by_geno = multcompare(rm, 'Condition', ...
                                    'By','geno_group', ...
                                    'ComparisonType','dunn-sidak');

    % 3) Genotype comparisons within each condition
