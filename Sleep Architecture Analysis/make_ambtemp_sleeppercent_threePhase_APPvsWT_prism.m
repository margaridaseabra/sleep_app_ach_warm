function OUT = make_ambtemp_sleeppercent_threePhase_APPvsWT_prism(rows_perhr, out_dir, states_to_plot, varargin)
% make_ambtemp_sleeppercent_threePhase_APPvsWT
% -------------------------------------------------------------------------
% For ambtemp recordings only:
%
%   For each requested STATE (e.g. WK, MA, NREM, REM, SLEEP):
%
%     Phase 1 (0–3 h):  hour_idx in [0 1 2]
%     Phase 2 (3–6 h):  hour_idx in [3 4 5]
%     Washout:          all remaining ambtemp hours (hour_idx not in above)
%
...
%           ambtemp_sleeppercent_0to3h_forPrism.csv
%           ambtemp_sleeppercent_3to6h_forPrism.csv
%           ambtemp_sleeppercent_washout_forPrism.csv
%
%       Layout:
...
if isempty(P)
    warning('No ambtemp rows in rows_perhr. Nothing to do.');
...
for s = 1:nStates
    ...
    title(sprintf('ambtemp: %%time in %s, 3 phases (WT vs APP)', st_upper));
    set(gca,'Box','off');
    ...
    fname    = sprintf('ambtemp_sleeppercent_threePhase_%s_APPvsWT%s.png', lower(st_upper), id_suffix);
...
for ph = 1:nPhases
    ...
    switch ph
        case 1
            tag  = 'h0_3';
            base = 'ambtemp_sleeppercent_0to3h_forPrism.csv';
        case 2
            tag  = 'h3_6';
            base = 'ambtemp_sleeppercent_3to6h_forPrism.csv';
        otherwise
            tag  = 'washout';
            base = 'ambtemp_sleeppercent_washout_forPrism.csv';
    end
