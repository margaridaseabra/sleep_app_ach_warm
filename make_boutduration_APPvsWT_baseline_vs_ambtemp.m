...
% -------------------------------------------------------------------------
% Here PERHOUR is the OVERALL table from
% run_group_sleep_architecture_ambtemp_overall, with columns:
...
%   mean_bout_dur = sum(total_dur_s) / sum(n_bouts)
%
% Plot (single figure):
...
% unify naming in case you have "ambtemp" / "ambtemp" / "washout"
PH.condition = replace(PH.condition, "ambtemp", "ambtemp");
PH.condition = replace(PH.condition, "washout", "ambtemp");

keep_cond    = PH.condition == "baseline" | PH.condition == "ambtemp";
PH           = PH(keep_cond, :);

if isempty(PH)
    warning('No baseline/ambtemp rows in input table. Nothing to plot.');
...
if ~all(ismember(needed, PH.Properties.VariableNames))
    error('Input table missing required columns for make_boutduration_APPvsWT_baseline_vs_ambtemp.');
end
...
for i = 1:nStates
    ...
    if numel(y_all) >= minNperGroupForStats * 2 && ...
       numel(unique(Ti.geno)) >= 2 && ...
       numel(unique(Ti.condition)) >= 2

        try
            gFactor = categorical(Ti.geno);
            cFactor = categorical(Ti.condition);
            [p, ~, ~] = anovan(y_all, ...
                {gFactor, cFactor}, ...
                'model','interaction', ...
                'display','off', ...
    ...
    n_WT_ambtemp(i)      = nVals(i,idxD,idxWT);
    n_APP_baseline(i)  = nVals(i,idxB,idxAPP);
    n_APP_ambtemp(i)     = nVals(i,idxD,idxAPP);

    mean_WT_baseline(i)   = meanVals(i,idxB,idxWT);
    mean_WT_ambtemp(i)      = meanVals(i,idxD,idxWT);
    mean_APP_baseline(i)  = meanVals(i,idxB,idxAPP);
    mean_APP_ambtemp(i)     = meanVals(i,idxD,idxAPP);

    sd_WT_baseline(i)   = sdVals(i,idxB,idxWT);
    sd_WT_ambtemp(i)      = sdVals(i,idxD,idxWT);
    sd_APP_baseline(i)  = sdVals(i,idxB,idxAPP);
    sd_APP_ambtemp(i)     = sdVals(i,idxD,idxAPP);
end
...
% Y-limits & star positioning (interaction p for stars)
