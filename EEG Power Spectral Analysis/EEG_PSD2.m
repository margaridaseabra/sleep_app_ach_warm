...
% Extract EEG and sampling frequency
EEG = eegData.eeg;
Fs = eegData.eeg_frequency;

% Extract sleep scoring data
TT = scoreData.TT;       % Timetable with state annotations
C = scoreData.C;         % Contains state codes
score = TT{:,1};         % Convert timetable to numeric score vector
...
% Define frequency bands (split lGamma to exclude 50Hz)
...
for s = 1:length(stateNames)
    ...
    for i = 1:length(epochs)
        ...
        if endIdx <= length(EEG)
            ...
            if isempty(F_ref)
                F_ref = F;  % Set frequency reference on first epoch
            end

            if length(pxx) ~= length(F_ref)
                warning('Skipping epoch %d due to mismatched PSD size.', i);
                continue;
            end
