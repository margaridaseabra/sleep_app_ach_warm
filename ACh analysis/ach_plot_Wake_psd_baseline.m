function ach_plot_Wake_psd_baseline(GROUP, out_dir, show_mouse_ids)
% Plot baseline Wake ACh PSD (WT vs APP) and bar plots for:
%   - Wake_power
%   - Wake_peakHz
%   - Wake_peakAmp
%   - Wake_cycleHz (ACh cycles in Hz)
%   - slope_Wake (ACh slope)
%
% INPUTS:
...
y_WT_power    = M.Wake_power(M.geno==genoWT);
y_APP_power   = M.Wake_power(M.geno==genoMut);
y_WT_peakHz   = M.Wake_peakHz(M.geno==genoWT);
y_APP_peakHz  = M.Wake_peakHz(M.geno==genoMut);
y_WT_peakAmp  = M.Wake_peakAmp(M.geno==genoWT);
y_APP_peakAmp = M.Wake_peakAmp(M.geno==genoMut);

% ACh cycle frequency (if available)
if ismember('Wake_cycleHz', M.Properties.VariableNames)
    y_WT_cycleHz  = M.Wake_cycleHz(M.geno==genoWT);
    y_APP_cycleHz = M.Wake_cycleHz(M.geno==genoMut);
else
    y_WT_cycleHz  = [];
    y_APP_cycleHz = [];
end
...
if ismember('slope_Wake', M.Properties.VariableNames)
    y_WT_slope  = M.slope_Wake(M.geno==genoWT);
    y_APP_slope = M.slope_Wake(M.geno==genoMut);
else
    y_WT_slope  = [];
    y_APP_slope = [];
end
...
% ---------- figure ----------
fig = figure('Name','Baseline Wake ACh PSD','Color','w','Position',[100 100 1680 300]);
...
if ~isempty(y_WT_slope) || ~isempty(y_APP_slope)
    plot_two_group_bar(y_WT_slope, y_APP_slope, genoWT, genoMut, ...
                       COL_WT, COL_APP, 'Wake slope (ΔF/F/s)', ...
                       show_mouse_ids, M(M.geno==genoWT,:), M(M.geno==genoMut,:));
else
...
sgtitle('Baseline Wake ACh – PSD & metrics');

if ~isempty(out_dir)
    if ~exist(out_dir,'dir'); mkdir(out_dir); end
    saveas(fig, fullfile(out_dir, 'Baseline_Wake_ACh_PSD.png'));
end
end
