function ach_plot_nrem_psd_temperature(GROUP, out_dir, show_mouse_ids, time_label)
% Plot NREM ACh PSD comparing baseline vs ambtemp (WT vs APP) with 2-way ANOVA
%
% INPUTS:
...
y_WT_BASE_power = M.NREM_power(M.geno == genoWT & M.cond == baseline_cond);
y_WT_AMB_power = M.NREM_power(M.geno == genoWT & M.cond == ambtemp_cond);
y_APP_BASE_power = M.NREM_power(M.geno == genoMut & M.cond == baseline_cond);
y_APP_AMB_power = M.NREM_power(M.geno == genoMut & M.cond == ambtemp_cond);

y_WT_BASE_peakHz = M.NREM_peakHz(M.geno == genoWT & M.cond == baseline_cond);
y_WT_AMB_peakHz = M.NREM_peakHz(M.geno == genoWT & M.cond == ambtemp_cond);
y_APP_BASE_peakHz = M.NREM_peakHz(M.geno == genoMut & M.cond == baseline_cond);
y_APP_AMB_peakHz = M.NREM_peakHz(M.geno == genoMut & M.cond == ambtemp_cond);

y_WT_BASE_peakAmp = M.NREM_peakAmp(M.geno == genoWT & M.cond == baseline_cond);
y_WT_AMB_peakAmp = M.NREM_peakAmp(M.geno == genoWT & M.cond == ambtemp_cond);
y_APP_BASE_peakAmp = M.NREM_peakAmp(M.geno == genoMut & M.cond == baseline_cond);
y_APP_AMB_peakAmp = M.NREM_peakAmp(M.geno == genoMut & M.cond == ambtemp_cond);

% ACh cycle frequency (if available)
if ismember('NREM_cycleHz', M.Properties.VariableNames)
    y_WT_BASE_cycleHz = M.NREM_cycleHz(M.geno == genoWT & M.cond == baseline_cond);
    y_WT_AMB_cycleHz = M.NREM_cycleHz(M.geno == genoWT & M.cond == ambtemp_cond);
    y_APP_BASE_cycleHz = M.NREM_cycleHz(M.geno == genoMut & M.cond == baseline_cond);
    y_APP_AMB_cycleHz = M.NREM_cycleHz(M.geno == genoMut & M.cond == ambtemp_cond);
else
...
if ismember('slope_NREM', M.Properties.VariableNames)
    y_WT_BASE_slope = M.slope_NREM(M.geno == genoWT & M.cond == baseline_cond);
    y_WT_AMB_slope = M.slope_NREM(M.geno == genoWT & M.cond == ambtemp_cond);
    y_APP_BASE_slope = M.slope_NREM(M.geno == genoMut & M.cond == baseline_cond);
    y_APP_AMB_slope = M.slope_NREM(M.geno == genoMut & M.cond == ambtemp_cond);
else
...
% ---------- figure ----------
fig = figure('Name', 'Temperature NREM ACh PSD', 'Color', 'w', 'Position', [100 100 1680 300]);
...
legend({[char(genoWT) ' Baseline'], [char(genoWT) ' Ambtemp'], ...
       ...
       'Box', 'off', 'Location', 'northeast', 'FontSize', 9);
...
plot_four_group_bar(y_WT_BASE_power, y_WT_AMB_power, y_APP_BASE_power, y_APP_AMB_power, ...
    ...
    'NREM power (A.U.)', show_mouse_ids, mice_WT_BASE, mice_WT_AMB, mice_APP_BASE, mice_APP_AMB);

subplot(1, 6, 3);
plot_four_group_bar(y_WT_BASE_peakHz, y_WT_AMB_peakHz, y_APP_BASE_peakHz, y_APP_AMB_peakHz, ...
    genoWT, genoMut, COL_WT_BASE, COL_WT_AMB, COL_APP_BASE, COL_APP_AMB, ...
    'NREM peak freq (Hz)', show_mouse_ids, mice_WT_BASE, mice_WT_AMB, mice_APP_BASE, mice_APP_AMB);

subplot(1, 6, 4);
plot_four_group_bar(y_WT_BASE_peakAmp, y_WT_AMB_peakAmp, y_APP_BASE_peakAmp, y_APP_AMB_peakAmp, ...
    genoWT, genoMut, COL_WT_BASE, COL_WT_AMB, COL_APP_BASE, COL_APP_AMB, ...
    'NREM peak amp (A.U.)', show_mouse_ids, mice_WT_BASE, mice_WT_AMB, mice_APP_BASE, mice_APP_AMB);
...
if ~isempty([y_WT_BASE_slope; y_WT_AMB_slope; y_APP_BASE_slope; y_APP_AMB_slope])
    plot_four_group_bar(y_WT_BASE_slope, y_WT_AMB_slope, y_APP_BASE_slope, y_APP_AMB_slope, ...
        genoWT, genoMut, COL_WT_BASE, COL_WT_AMB, COL_APP_BASE, COL_APP_AMB, ...
        'NREM slope (ΔF/F/s)', show_mouse_ids, mice_WT_BASE, mice_WT_AMB, mice_APP_BASE, mice_APP_AMB);
else
    text(0.5, 0.5, 'No slope data', 'HorizontalAlignment', 'center');
    axis off;
end

sgtitle(['Temperature Comparison – NREM ACh PSD & metrics – ' time_label]);
...
if ~isempty(out_dir)
    ...
    fname = sprintf('Temperature_NREM_ACh_PSD_%s.png', strrep(time_label, ' ', '_'));
