function ach_plot_wake_psd_temperature2(GROUP, out_dir, show_mouse_ids, time_label)
...
fprintf('\n=== Temperature PSD Comparison (Wake) ===\n');
...
sgtitle(['Temperature Comparison – Wake ACh PSD & metrics – ' time_label]);
...
if ~isempty(out_dir)
    ...
    fname = sprintf('Temperature_Wake_ACh_PSD_%s.png', strrep(time_label, ' ', '_'));
...
% Scatter points with optional labels
jitter = 0.08;
...
if sum(ok) > 3 && numel(unique(cond_factor(ok))) > 1 && numel(unique(geno_factor(ok))) > 1
    ...
    % Display p-values compactly at top-right
...
if show_ids && ~isempty(mouse_ids)
    for i = 1:numel(y_vals)
        if i <= numel(mouse_ids)
            % Position text slightly to the right and use smaller font
            text(xpos_jittered(i) + 0.08, y_vals(i), mouse_ids{i}, ...
