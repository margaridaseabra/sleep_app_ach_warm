function ach_plot_wake_psd_three_conditions(GROUP, out_dir, show_mouse_ids, time_label)
% Plot Wake ACh PSD comparing baseline vs ambtemp vs drugs (WT vs APP) with 2-way ANOVA
...
drugs_cond = all_conds(contains(lower(all_conds), 'drugs') | contains(lower(all_conds), 'drugs'));
...
for k = 1:numel(sessions)
    ...
    P = [];
    if isfield(OUT.psd.Wake, 'Pxx'); P = OUT.psd.Wake.Pxx(:);
    elseif isfield(OUT.psd.Wake, 'psd'); P = OUT.psd.Wake.psd(:);
    elseif isfield(OUT.psd.Wake, 'power'); P = OUT.psd.Wake.power(:);
    end
...
metrics = {'Wake_power', 'Wake_peakHz', 'Wake_peakAmp', 'Wake_cycleHz', 'slope_Wake'};
data = struct();
mice = struct();
...
% Create figure
fig = figure('Name', 'Wake PSD - Three Conditions', 'Color', 'w', 'Position', [100 100 1680 300]);
...
xlabel('Frequency (Hz)'); ylabel('Power (A.U.)'); title('Wake ACh PSD'); box off;
...
% Bar plots
subplot(1, 6, 2);
if isfield(data, 'Wake_power')
    plot_six_group_bar(data.Wake_power, mice.Wake_power, genoWT, genoMut, ...
        COL_WT_BASE, COL_APP_BASE, COL_WT_AMB, COL_APP_AMB, COL_WT_DRUGS, COL_APP_DRUGS, ...
        'Wake power (A.U.)', show_mouse_ids);
end

subplot(1, 6, 3);
if isfield(data, 'Wake_peakHz')
    plot_six_group_bar(data.Wake_peakHz, mice.Wake_peakHz, genoWT, genoMut, ...
        COL_WT_BASE, COL_APP_BASE, COL_WT_AMB, COL_APP_AMB, COL_WT_DRUGS, COL_APP_DRUGS, ...
        'Wake peak freq (Hz)', show_mouse_ids);
end

subplot(1, 6, 4);
if isfield(data, 'Wake_peakAmp')
    plot_six_group_bar(data.Wake_peakAmp, mice.Wake_peakAmp, genoWT, genoMut, ...
        COL_WT_BASE, COL_APP_BASE, COL_WT_AMB, COL_APP_AMB, COL_WT_DRUGS, COL_APP_DRUGS, ...
        'Wake peak amp (A.U.)', show_mouse_ids);
end

subplot(1, 6, 5);
if isfield(data, 'Wake_cycleHz')
    plot_six_group_bar(data.Wake_cycleHz, mice.Wake_cycleHz, genoWT, genoMut, ...
        COL_WT_BASE, COL_APP_BASE, COL_WT_AMB, COL_APP_AMB, COL_WT_DRUGS, COL_APP_DRUGS, ...
...
subplot(1, 6, 6);
if isfield(data, 'slope_Wake')
    plot_six_group_bar(data.slope_Wake, mice.slope_Wake, genoWT, genoMut, ...
        COL_WT_BASE, COL_APP_BASE, COL_WT_AMB, COL_APP_AMB, COL_WT_DRUGS, COL_APP_DRUGS, ...
        'Wake slope (ΔF/F/s)', show_mouse_ids);
else
    text(0.5, 0.5, 'No slope data', 'HorizontalAlignment', 'center'); axis off;
end

sgtitle(['Three Conditions – Wake ACh PSD & metrics – ' time_label]);
...
if ~isempty(out_dir)
    ...
    saveas(fig, fullfile(out_dir, sprintf('Wake_PSD_ThreeCond_%s.png', strrep(time_label, ' ', '_'))));
end
end
